...
# Written by Yiting Wang  2021.03.09
# ------------------------------------------------------------------------------
...
# from https://github.com/pytorch/vision/blob/master/torchvision/models/shufflenetv2.py
...
class ShuffleNetV2(nn.Module):
    ...
    def _forward_impl_part(self, x: Tensor) -> Tensor:
        
        return x

    def forward(self, x: Tensor) -> Tensor:
        #return self._forward_impl(x)
        return self._forward_impl_part(x)
...
class PoseShuffleNetShort(nn.Module):
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        for i in range(num_layers):
            ...
            layers.append(nn.Conv2d(planes, planes, 1, 1, 0, bias=False))
            layers.append(nn.BatchNorm2d(planes, momentum=BN_MOMENTUM))
            layers.append(nn.ReLU(inplace=True))
