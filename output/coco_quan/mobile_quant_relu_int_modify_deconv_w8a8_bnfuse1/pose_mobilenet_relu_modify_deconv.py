...
# Written by Yiting Wang  2021.03.27
# ------------------------------------------------------------------------------
...
class PoseMobileNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, 
        ...
        **kwargs ) -> None:
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(input_channel, self.inplanes, kernel_size=1, stride=1, padding=0, bias=False), 
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):  # Idea comes from Mobilenet(depthwise separable convolution)
        ...
        for i in range(num_layers):
            ...
            if(i!=0):
                layers.append(nn.Conv2d(64, 128, 1, 1, 0, bias=False)) # pointwise convolution   nn.conv( in_channels: int,out_channels: int,...)
            ...
            layers.append(nn.Conv2d(128, planes, 1, 1, 0, bias=False)) # pointwise convolution
