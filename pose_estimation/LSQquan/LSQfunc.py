...
class QuanConvTranspose2d(t.nn.ConvTranspose2d):
    def __init__(self, m: t.nn.ConvTranspose2d, quan_w_fn=None, quan_a_fn=None, dilation=1):
    ...
    def forward(self, x):
        quantized_weight = self.quan_w_fn(self.weight)
        quantized_act = self.quan_a_fn(x)
        return t.nn.functional.conv_transpose2d(quantized_act, quantized_weight, self.bias, self.stride, self.padding, self.output_padding, self.groups, self.dilation)
