...
def parse_args():
    ...
    # general
    parser.add_argument('--cfg', type=str, default='experiments/coco/resnet50/256x192_d256x3_adam_lr1e-3_mobile.yaml') #256x192_d256x3_adam_lr1e-3_mobile.yaml
    ...
    parser.add_argument('--image_dir',type=str,default='demo/test/')#, default='demo/test_val2017/')
...
def main():
    ...
    pose_model = eval('models.'+cfg.MODEL.NAME+'.get_pose_net')(
        ...
        is_train=False,
        int_adjust=False, #False是float版本，True是int版本（默认）
    )

    if cfg.TEST.MODEL_FILE:
        print('=> loading model from {}'.format(cfg.TEST.MODEL_FILE))
        pose_model.load_state_dict(torch.load(cfg.TEST.MODEL_FILE)['model'], strict=True) #strict=False时，相当于都没把权重读入
    else:
    ...
    if args.webcam or args.video:
        ...
        for i in range(index):
            ...
            input = []
            img = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
            img_tensor = torch.from_numpy(img/255.).permute(2,0,1).float().to(CTX)  #维度替换
            input.append(img_tensor)
            ...
            if args.write:
                save_path = 'demo/mobilenetv2/test_image_det_'+str(i)+'.jpg'
